...
public class AIControllerBT_V2 : BehaviorTree
{
    [Header("FIGHTER")] 
    ...
    private Skill m_CurrentSkillInUse;
    ...
    public override void Start()
    {
        ...
        roamingTask.m_OriginalPosition = GetComponent<Mover>().GetInitialPosition();
        roamingTask.m_Fighter = m_Fighter;
        roamingTask.m_TimeBetweenNewDestination = m_TimeBetweenNewDestination;
        //roamingTask.m_Party = GetComponentInParent<PartySystem>();
        ...
        //splitRoamingFollowLeader.m_Party = GetComponentInParent<PartySystem>();

        FollowLeader_Task followLeaderTask = new FollowLeader_Task(this);
        followLeaderTask.m_Transform = transform;
        //followLeaderTask.m_Party = GetComponentInParent<PartySystem>();
        ...
        followLeaderTask.m_OriginalPosition = GetComponent<Mover>().GetInitialPosition();
        ...
        selectTarget.children.Add(skillUsage);

        selectTargetToSupport.children.Add(moveToTargetToSupport);

        //moveToTarget.children.Add(splitTask);

        supportiveSkillUsage.children.Add(skillUsage);

        moveToTargetToSupport.children.Add(supportiveSkillUsage);

        skillUsage.children.Add(moveToTarget);
        //weaponAttack.children.Add(moveToTarget);
        //splitTask.children.Add(skillUsage);
        //splitTask.children.Add(weaponAttack);
        ...
        SetValue("CurrentSkillInUse", m_CurrentSkillInUse);
    ...
    private void ResetMoveTargetTaskVariables(int depth = 0)
    {
        if (depth == 0)
        {
            ...
            //List<GameObject> allyTargets = (List<GameObject>)GetValue("PotentialAllyTargets");
            //allyTargets.Clear();
            //SetValue("PotentialAllyTargets", allyTargets);
            //List<GameObject> alliesInNeed = (List<GameObject>)GetValue("AlliesInNeed");
            //alliesInNeed.Clear();
            //SetValue("PotentialAllyTargets", alliesInNeed);
            SetValue("m_Requested", false);
            SetValue("ClosestTarget", null);
            m_Fighter.m_Target = null;
        }
